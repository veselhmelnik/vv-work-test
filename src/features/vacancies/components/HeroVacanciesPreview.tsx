...
import { HeroVacanciesSkeleton } from './HeroVacanciesSkeleton'
...
export function HeroVacanciesPreview() {
  ...
  if (error) {
    return <ErrorState onRetry={retry} title="Не вдалося завантажити вакансії"/>
  }
    ...
    <div className="rounded-2xl border border-border bg-surface p-4 shadow-sm md:rounded-3xl md:p-6">
      <div className="flex items-start justify-between gap-3">
        ...
        <span className="shrink-0 rounded-full bg-dark px-2.5 py-1 text-xs text-white md:px-3 md:text-sm">
          {formatVacanciesCount(vacancies.length)}
        </span>
      </div>
          ...
          >
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                ...
                <h3 className="mt-1 truncate font-semibold">{vacancy.title}</h3>

                <p className="mt-1 truncate text-sm text-muted">
                  {vacancy.company} · {vacancy.city}
                </p>

                <p className="mt-2 text-sm font-semibold">{vacancy.salary}</p>
              </div>
